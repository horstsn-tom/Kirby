...
- **Rendering** — `drawRoom()`, `drawVisitor()`, and `drawHud()` are called each frame from `draw()`; `drawVisitor()` dispatches to `VISITOR_SPRITE[visitorType]` (`drawMomSprite()`/`drawBrotherSprite()`/`drawDadSprite()`). The reaction-time bar, warning glyph, and life indicators (grass-blade triangles) are drawn directly on the canvas, colored per `VISITOR_COLOR`, not as DOM/emoji elements. The warning-phase "!" glyph stops rendering once `state.score >= WARNING_ICON_HIDE_SCORE` (1000), a late-game difficulty bump — the door still opens and the warning message still plays, just without the visual heads-up.
