...
- **Input** — `handleAction(type)` is the single entry point for reacting; it only does anything when `visitPhase === 'approaching'`. A matching `type` succeeds (moves to `leaving`); a mismatched `type` (wrong button for the current visitor) calls `triggerCaught(true)`, the same life-losing transition a timeout uses (`triggerCaught(false)`) — `state.wrongInput` distinguishes the two for the caught-phase message. `keydown` maps `Tab` → `handleAction('mom')` and `KeyY` → `handleAction('brother')` (both `preventDefault`ed so Tab never shifts browser focus), plus `Space` → start/restart. The `#tabBtn`/`#yellBtn` on-screen buttons and a canvas tap (→ `Space`) mirror the same handlers for touch devices, so keep new input paths going through `handleAction`/`handleSpace`/`triggerCaught` rather than duplicating state changes.
